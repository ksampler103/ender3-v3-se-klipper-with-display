...
#if CONFIG_ENABLE_SERIAL_BRIDGE_USART1
    {0, USART1, GPIO('A', 3), GPIO('A', 2), 7, 7},
    {1, USART1, GPIO('B', 7), GPIO('B', 6), 7, 7},
#endif
#if CONFIG_ENABLE_SERIAL_BRIDGE_USART2
    {2, USART2, GPIO('D', 6), GPIO('D', 5), 7, 7},
    {3, USART2, GPIO('A', 3), GPIO('A', 2), 7, 7},
#endif
#if CONFIG_ENABLE_SERIAL_BRIDGE_USART6
    {4, USART6, GPIO('A', 12), GPIO('A', 11), 8, 8},
#endif
};
...
#if CONFIG_ENABLE_SERIAL_BRIDGE_USART1
void USART1_serial_bridge_IRQHandler(void)
{
...
#if CONFIG_ENABLE_SERIAL_BRIDGE_USART2
void USART2_serial_bridge_IRQHandler(void)
{
    serial_bridge_handle_uart_irq(
...
#if CONFIG_ENABLE_SERIAL_BRIDGE_USART6
void USART6_serial_bridge_IRQHandler(void)
{
    serial_bridge_handle_uart_irq(
...
int8_t serial_bridge_configure(uint8_t* config, uint32_t* baud)
{
    ...
    gpio_peripheral(s_config->rx_pin, GPIO_FUNCTION(s_config->rx_alt_function),
                    1);
    gpio_peripheral(s_config->tx_pin, GPIO_FUNCTION(s_config->tx_alt_function),
                    0);
