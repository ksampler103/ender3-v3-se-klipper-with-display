...
static void deal_dirs(struct dirzctl* d, int32_t dir, int32_t is_save)
{
    if (is_save > 0)
    {
...
static void deal_steps(struct dirzctl* d)
{
    for (int i = 0; i < d->count; i++)
        gpio_out_toggle_noirq(d->steps[i]);
}

static void send_status(struct dirzctl* d)
{
...
void command_config_dirzctl(uint32_t* args)
{
    sendf("debug_dirzctl oid=%c arg[0]=%u arg[1]=%u arg[2]=%u arg[3]=%u "
          "arg[4]=%u arg[5]=%u",
          (uint8_t)args[0], (uint32_t)args[0], (uint32_t)args[1],
...
DECL_COMMAND(command_add_dirzctl, "add_dirzctl oid=%c index=%c dir_pin=%u "
...
void command_run_dirzctl(uint32_t* args)
{
    ...
    d->n_ticks =
        (uint32_t)(((double)args[2] / 1000000) * CONFIG_CLOCK_FREQ) / 2;
    d->r_steps = args[3] * 2;
...
void dirzctl_task(void)
{
    ...
    foreach_oid(oid, d, command_config_dirzctl)
    {
        ...
        sendf("result_dirzctl oid=%c step=%u tick=%u", (uint8_t)d->oid,
              (uint16_t)d->a_steps, (uint32_t)d->a_ticks);
...
void dirzctl_shutdown(void)
{
    ...
    foreach_oid(oid, d, command_config_dirzctl) { d->a_finish = 0; }
